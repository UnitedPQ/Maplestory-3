...
(function($) {
	...
	$(document).ready(function(){
		...
		if( $("filter#slotMachineBlurSVG").length<=0 ){
			$("body").append('<div style="position:absolute;top:0;left:0;"><svg version="1.1" xmlns="http://www.w3.org/2000/svg">'+
	...
	$.fn.slotMachine = function(settings){
		...
		*/
		function _getRandom(){
			var rnd;
			if(_result > 0){
				rnd = $slot.find('[data-result='+_result+']').index();
			}else{
				do{
		...
		*/
		...
		/**
		  ...
		  * @param int count - Number of shuffles (undefined to make infinite animation
		*/
		$slot.shuffle = function( count, result, oncomplete ){
